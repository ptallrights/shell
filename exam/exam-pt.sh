...
[rhel]
name=localsoftware
baseurl=http://172.25.254.250/source
enabled=1
gpgcheck=0
end
...
lvcreate -L 512M -n lv0 vg0 &>/dev/null
mkfs.ext2 /dev/vg0/lv0 &>/dev/null
cp -p -r /home/* /dev/vg0/lv0 &>/dev/null
echo "/dev/vg0/lv0 /home ext2 defaults 0 0" >> /etc/fstab
mount -a &>/dev/null
if [ $? -eq 0 ];then
	echo "OK"
else
...
authconfig --enableldap --enableldaptls --enablekrb5 --ldaploadcacert=http://172.25.254.250/example-ca.crt --ldapserver=classroom.example.com --ldapbasedn="dc=example,dc=com" --krb5kdc=classroom.example.com --krb5adminserver=classroom.example.com --krb5realm=EXAMPLE.COM --updateall 
getent passwd ldapuseer1 &>/dev/null
if [ $? -eq 0 ];then
	echo "OK"
else
	echo "Error"
